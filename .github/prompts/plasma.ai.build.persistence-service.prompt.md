---
description: "Implements the persistence-service component (CRUD + versioning/draft) from the Persistence Services section of specs/services.md."
agent: agent
...
Call the `plasma_ai_build_persistence_service` tool. If the user provided extra scope or instructions below,
pass them as `context`; otherwise call it with an empty context to implement the full Persistence
Services section.

Then follow the instructions returned by the tool exactly, creating all generated files under
`packages/<prefix>-persistence-service/`. If the tool reports an error (missing `specs/services.md`),
